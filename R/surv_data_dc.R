...
#'
#' The generated dataset includes three varaibles, which are \eqn{X_{i}}, \eqn{\delta_{i}} and \eqn{\eta_i}, i.e. \eqn{X_i=\text{min}(T_i, C_i, A_i)},
#' \eqn{\delta_i=I(X_i=T_i)} and \eqn{\eta_i=I(X_i=C_i)}, for \eqn{i=1,\ldots,n}. 'T' represents the event time, whose hazard function is
#' \deqn{h_{T}(x)=h_{0T}(x)\text{exp}(\boldsymbol Z^{\top}\beta)}, where the baseline hazard can take weibull form, i.e. \eqn{h_{0T}(x) = ax^{a-1} / \lambda^a}, or
#' log logistic form, i.e.
#' \deqn{ h_{0T}(x) = \frac{ \frac{ 1 }{ a\text{exp}( \lambda ) } \left ( \frac{ x }{ \text{exp}( \lambda ) } \right)^{1/a -1 } }{ 1 + \left( \frac{ x }{ \text{exp}( \lambda ) } \right)^{1/a}  } }.
#' 'C' represents the dependent censoring time, whose hazard function is \eqn{ h_{C}(x) = h_{0C}(x)\text{exp}(\boldsymbol Z^{\top}\phi) }, where the baseline hazard can take exponential form, i.e.
